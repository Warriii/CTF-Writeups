...
void disown_a_kid() {
	if (kid_adopted && life.kidName) {
		...
		free(life.kidLastName); // frees same addr as life.LastName
		free(life.kidName); // 0x30 chunk sz suspect
		life.kidName = NULL;
...
void setup() {
	...
	life.lastName = calloc(0x20, sizeof(char)); // 0x30 chunk sz suspect
